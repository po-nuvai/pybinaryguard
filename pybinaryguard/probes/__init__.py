...
def get_all_probes() -> List[ProbeBase]:
    ...
    2. **CpuProbe** -- CPU architecture and flags (reads /proc/cpuinfo)
    3. **OsProbe** -- OS and container detection
    4. **GlibcProbe** -- C library version
    5. **LibraryProbe** -- shared library paths and ldconfig cache
    6. **BoardProbe** -- embedded board detection
    7. **GpuProbe** -- GPU / CUDA detection (most expensive)
    """
    return [
        PythonProbe(),
        CpuProbe(),
